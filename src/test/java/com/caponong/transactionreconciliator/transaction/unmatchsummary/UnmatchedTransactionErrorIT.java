...
public class UnmatchedTransactionErrorIT extends ApplicationTest {
    ...
    public void testInvalidReconciliationToken() throws Exception {
        mockMvc.perform(
                get(BASE_PATH + "extra" + TEST_RECONCILIATION_TOKEN + TRANSACTION_UNMATCHED_SUMMARY_API)
                        .servletPath(BASE_PATH + "extra" + TEST_RECONCILIATION_TOKEN + TRANSACTION_UNMATCHED_SUMMARY_API))
    ...
    public void testRequestEncounteredAnError() throws Exception {
        ...
        mockMvc.perform(
                get(BASE_PATH + TEST_RECONCILIATION_TOKEN2 + TRANSACTION_UNMATCHED_SUMMARY_API)
                        .servletPath(BASE_PATH + TEST_RECONCILIATION_TOKEN2 + TRANSACTION_UNMATCHED_SUMMARY_API))
                .andExpect(status().isBadRequest())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.code", is("999")))
                .andExpect(jsonPath("$.message", is("error")))
    ...
    public void testRequestTokenNotFound() throws Exception {
        ...
        mockMvc.perform(
                get(BASE_PATH + TEST_RECONCILIATION_TOKEN3 + TRANSACTION_UNMATCHED_SUMMARY_API)
                        .servletPath(BASE_PATH + TEST_RECONCILIATION_TOKEN3 + TRANSACTION_UNMATCHED_SUMMARY_API))
